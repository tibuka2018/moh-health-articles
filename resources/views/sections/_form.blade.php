<div class="form-group{{ $errors->has('title') ? ' has-error ' : '' }} has-feedback"> 
	...
	<div class="col-sm-10">
		<input type="text" name="title" value="{{ isset($section->title) ? $section->title : old('title') }}" id="title" class="form-control" placeholder="Title" aria-describedby="titleErrorStatus">
		@if($errors->has('title'))
...
<div class="form-group{{ $errors->has('content') ? ' has-error ' : '' }} has-feedback">
	...
	<div class="col-sm-10">
		<textarea name="content" id="summernote" rows="10" class="form-control" aria-describedby="contentErrorStatus">{{ isset($section->content) ? $section->content : old('content') }}</textarea>
